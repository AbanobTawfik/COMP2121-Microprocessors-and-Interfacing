...
star:
    ...
    out portA, temp
    ldi temp, DOOR_CLOSED                             ; reset the LED state to show elevator is now taking requests
    out portC, temp
...
somethinginQueue:
    ...
    brne checkk                                       ; if the current floor is not equal to the queue we go to the check 
...
nothinginQueue:
    ldi temp1, 0
    sts secondCounter, temp1                          ; reset the second counter when nothing in queue
    ldi temp1, 1
    sts canOpen, temp1                                ; set the flag for door can open since its at a floor
    lds temp1, opening                                ; now we check if user has pressed open button 
    cpi temp1, 1
    breq addCurrToQueue                               ; if user has pressed open button add the current floor to queue
    jmp timerEpilogue
addCurrToQueue:
    ldi temp1, 0                                      ; clear the open button so it doesnt infinitely reopen
    sts opening, temp1
    lds temp1, currentFloor                           ; now we want to convert current floor into bit representation
    ldi XL, 1
    ldi XH, 0 
    CONVERT_FLOOR_INTEGER temp1, XL, XH
    lds ZL, floor_Queue                               ; now we want to add the floor to queue using our macro
    lds ZH, floor_Queue + 1                           ; see macro for description
    UPDATE_STATE_ADD ZL,XL,ZH,XH
    sts floor_Queue, ZL
    sts floor_Queue+1,ZH
timerEpilogue:
    epilogue                                          ; return from interrupt handler
    reti
; This entire function will be responsible for opening the door and removing the floor from the queue
OPEN_DOOR:
    prologue
	ldi temp, 1
	sts doorSequence, temp                            ; first we enable flag that the door sequence has begun (for emergency)
    lds temp, secondCounter
    cpi temp, 3                                       ; if 1 second has passed since we reached floor (2s) we want to go to open door state
    brsh skipOpen
    ldi temp1, 0xff                                   ; otherwise we keep the motor running for one second
    sts OCR3BL, temp1                                 ; setting motor to full speed
    sts OCR3BH, temp1
    ldi temp1, DOOR_CLOSED                            ; output to portC that the door is closed pattern 0b11100111
    out portC, temp1
    jmp TimerEpilogue                                 ; return from interrupt handler
; if the door has opened for 1s now we want to keep open for 3s
skipOpen:
    ldi temp1, DOOR_OPEN                              ; output the open door pattern 0b11000011
    out portC, temp1    
    ldi temp1, 1
    sts canClose, temp1                               ; set the canClose flag which notifies that the door is able to be closed
    ldi temp1, 0
    sts OCR3BL, temp1                                 ; supply no voltage to motor when door closed (motor off)
    sts OCR3BH, temp1
    lds temp, closing                                 ; now we want to check if the close button was pushed while door was open
    cpi temp, 1
    breq startClosing                                 ; if the flag for close was set we want to start closing immediately
    lds temp, secondCounter
    cpi temp, 7                                       ; otherwise wait till 7s has passed since lift started moving 2s for floor 1s for open 3s for stay open
    brsh startClosingcheck                            ; if the door has been open for 3s we want to try to start closing
    jmp TimerEpilogue
startclosingcheck:
    in temp, pinD                                     ; now we want to check pinD if the open button was held down
    ldi temp1, 0b0000001
    and temp1, temp                                   ; PB1 is connected to RDX3 with bit 0b00000001
    cpi temp1, 0                                      ; when we and together we want to check if the result is 0
    breq stayopen                                     ; if result is = 0 that means the pin is low -> button is still held down
    jmp startClosing                                  ; otherwise we start closing
; this will keep the door open
stayopen:
    ldi temp, 7                                       ; this will set the time to 7 so it will loop to this part over and over till button is let go
    sts secondCounter, temp                           ; store 7s into time
    jmp TimerEpilogue                                 ; return from interrupt
; start closing the door
startClosing:                              
    ldi temp1, 1
    sts canOpen, temp1                                ; set the can open flag for pb1 to keep door open 
    lds temp, opening
    cpi temp, 1
    breq startReopening                               ; if the button was pushed to open, we want to start re-opening
    jmp skipReopening
startReopening:
    ldi temp, 4
    sts secondCounter, temp                           ; this will set the time to 4 so 3 more seconds have to pass to start closing again
    ldi temp, 0
    sts opening, temp                                 ; clear the opening flag to stop the infinite loop
    epilogue
    ret                                               ; return from interrupt
; otherwise we aren't re-opening we want to close the door
skipReopening:
    ldi temp1, DOOR_CLOSED
    out portC, temp1                                  ; now we want to turn motor ON to signify closing door
    ldi temp1, 0xff
    sts OCR3BL, temp1
    sts OCR3BH, temp1                                 ; supply maximum voltage to the motor to spin at full speed
    lds temp, closing                                 
    cpi temp, 1                                       ; this will check if the close button has been pressed 
    breq closecheck                                   ; if it has we want to go to the special procedure 
    jmp skipovercheck                                 ; otherwise continue to close normally
...
; then close the door
closecheck:
    lds temp, alreadyClosing                          ; check if the door is already closing with the motor
    cpi temp, 1
    breq closeWithMotor                               ; if the door is already closing with the motor skip over initialises  
    lds temp, secondCounter                            
    inc temp                                          ; otherwise we want to load and increment the current second counter
    sts timeOfClose, temp                             ; and set that to the time we want before the door is closed so door closes with motor for 1s
    lds temp, secondCounter
    sts secondCounter, temp
    jmp closeWithMotor                                ; now we want to close door with motor with button press
; otherwise we want to wait 1s normally for door to close
skipovercheck:
    lds temp, secondCounter
    cpi temp, 9                                       ; check 9s has passed before we set door shut (2s for moving 1s for opening 3s for waiting 1s for closing)
    brsh closed                                       ; if it is go to shut the door protocol
    jmp TimerEpilogue                                 ; return from interrupt otherwise
...
closeWithMotor:
    ldi temp1, 1
    sts alreadyClosing, temp1                         ; set the already closing flag for when we return through open door process 
    ldi temp1, 0xff
    sts OCR3BL, temp1                                 ; supply max voltage to motor so motor spins at full speed to show closing
    sts OCR3BH, temp1
    lds temp1, timeOfClose
    lds temp2, secondCounter
    cp temp2, temp1                                   ; now we want to compare the time the button was pressed with that time + 1
	                                                  ; to check if one second has passed
    brsh closed                                       ; if one second has passed -> SHUT THE DOOR
    jmp TimerEpilogue                                 ; otherwise return from interrupt handler
; this will be used to shut the door
closed:
    ldi temp1, 0
    sts OCR3BL, temp1                                 ; now we want to turn off motor to show door is closed  
    sts OCR3BH, temp1
    sts secondCounter, temp1                          ; reset ALL FLAGS + TIMERS 
    sts closing, temp1
	...
	lds temp, currentFloor                            ; convert the current floor into the bit representation
    ldi XL, 1
    ldi XH, 0 
    CONVERT_FLOOR_INTEGER temp, XL, XH
    lds ZL, floor_Queue
    lds ZH, floor_Queue + 1                           ; load the queue from label 
    eor ZL,XL                                         ; now we want to exclusive or the current floor with queue
    eor ZH,XH                                         ; this will remove the floor example below
    ...
    sts floor_Queue, ZL
    sts floor_Queue+1,ZH                              ; store the updated queue into the label
    lds temp, emergency                               ; now we want to check if the emergency was pressed
    cpi temp, 1
    breq showDisplay                                  ; if the emergency flag is set we want to go to emergency protocol
    jmp skipDisplay                                   ; otherwise we want to just return
showDisplay:
	lds temp, CurrentFloor
	cpi temp, 0                                       ; now we want to check if we are on floor 0 before we display emergency command
	breq cont
	epilogue                                          ; otherwise return as normal
	ret
; if the emergency flag is on and floor is 0
cont:
    ldi temp, 1
    sts emergencyshown, temp                          ; set flag for emergency shown so lcd will keep displaying emergency
    ldi temp, 0xff                                    ; output a full lock pattern to LED all leds on
    out portC, temp
    PRINT_EMERGENCY                                   ; display emergency message
    epilogue                                          ; return
    ret
skipDisplay:
    epilogue                                          ; return 
...
forceClose:
    prologue
    ldi temp1, 0xff
    sts OCR3BL, temp1                                 ; supply max voltage to motor to signify motor is closing
    sts OCR3BH, temp1    
    rcall sleep_1s                                    ; close for 1 second can use sleep since we aren't taking requests
    ldi temp1, DOOR_CLOSED
    out portC, temp1                                  ; output closed door pattern to LEDS
    ldi temp1, 0                                  
    sts OCR3BL, temp1
    sts OCR3BH, temp1                                 ; supply no more voltage to motor to signify motor off door is shut
    sts secondCounter, temp1                          ; RESET ALL flags and counters
    sts closing, temp1
    sts timeOfClose, temp1
    sts alreadyClosing, temp1
    sts canClose, temp1
	...
	sts doorSequence, temp1
	; now we want to remove the floor from the queue     
	lds temp, currentFloor                            ; convert current floor into bit representation
    ldi XL, 1
    ldi XH, 0 
    CONVERT_FLOOR_INTEGER temp, XL, XH
    lds ZL, floor_Queue                               ; load the queue from memory
    lds ZH, floor_Queue + 1    
    eor ZL,XL                                         ; now we want to EOR which will remove the current floor from the queue
    eor ZH,XH
    sts floor_Queue, ZL                               ; store updated queue into label
    sts floor_Queue+1,ZH
